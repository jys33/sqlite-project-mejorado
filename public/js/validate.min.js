 ...
 * H=/\d{4}-\d{1,2}-\d{1,2}/
 */
(function(r,t,l){var u={required:"El campo %s es requerido.",matches:"El campo %s no coincide con el campo %s.","default":"El campo %s todavía está configurado como predeterminado, cambie.",valid_email:"El campo %s debe contener una dirección de correo electrónico válida.",valid_emails:"El campo %s debe contener todas las direcciones de correo electrónico válidas.",min_length:"El campo %s debe tener al menos %s caracteres de longitud.",max_length:"El campo %s no debe exceder los %s caracteres de longitud.",exact_length:"El campo %s debe tener exactamente la longitud de %s caracteres.",
...
is_natural:"El campo %s debe contener solo números positivos.",is_natural_no_zero:"El campo %s debe contener un número mayor que cero.",valid_ip:"El campo %s debe contener una IP válida.",valid_base64:"El campo %s debe contener una cadena base64.",valid_credit_card:"El campo %s debe contener un número de tarjeta de crédito válido.",is_file_type:"El campo %s debe contener solo archivos %s.",valid_url:"El campo %s debe contener una URL válida.",greater_than_date:"El campo %s debe contener una fecha más reciente que %s.",less_than_date:"El campo %s debe contener una fecha anterior a %s.",
greater_than_or_equal_date:"El campo %s debe contener una fecha que sea al menos tan reciente como %s.",less_than_or_equal_date:"El campo %s debe contener una fecha que sea %s o más antigua."},v=function(a){},w=/^(.+?)\[(.+)\]$/,g=/^[0-9]+$/,x=/^\-?[0-9]+$/,m=/^\-?[0-9]*\.?[0-9]+$/,/*SE AGREGO NUEVA EXPRESIÓN REGULAR PARA EL EMAIL*/q=/^[_\.0-9a-zA-Z-]+@([0-9a-zA-Z][0-9a-zA-Z-]+\.)+[a-zA-Z]{2,6}$/i,y=/^[a-záéíóúÁÉÍÓÚÑñÜü ]+$/i,/*SE AGREGO CARACTERES CON ACENTO*/z=/^[a-z0-9]+$/i,A=/^[a-z0-9_\-]+$/i,B=/^[0-9]+$/i,C=/^[1-9][0-9]*$/i,D=/^((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){3}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})$/i,
E=/[^a-zA-Z0-9\/\+=]/i,F=/^[\d\-\s]+$/,G=/^((http|https):\/\/(\w+:{0,1}\w*@)?(\S+)|)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?$/,H=/\d{1,2}-\d{1,2}-\d{4}/,f=function(a,c,b){this.callback=b||v;this.errors=[];this.fields={};this.form=this._formByNameOrNode(a)||{};this.messages={};this.handlers={};this.conditionals={};a=0;for(b=c.length;a<b;a++){var d=c[a];if((d.name||d.names)&&d.rules)if(d.names)for(var n=0,f=d.names.length;n<f;n++)this._addField(d,d.names[n]);else this._addField(d,d.name);else console.warn("validate.js: El siguiente campo se está omitiendo debido a una configuración incorrecta:"),
console.warn(d),console.warn("Verifique que haya configurado correctamente un nombre y reglas para este campo")}var e=this.form.onsubmit;this.form.onsubmit=function(a){return function(b){try{return a._validateForm(b)&&(e===l||e())}catch(c){}}}(this)},p=function(a,c){var b;if(0<a.length&&("radio"===a[0].type||"checkbox"===a[0].type))for(b=0,elementLength=a.length;b<elementLength;b++){if(a[b].checked)return a[b][c]}else return a[c]};f.prototype.setMessage=function(a,c){this.messages[a]=c;return this};
...
typeof this._hooks[e]?this._hooks[e].apply(this,[a,b])||(h=!0):"callback_"===e.substring(0,9)&&(e=e.substring(9,e.length),"function"===typeof this.handlers[e]&&!1===this.handlers[e].apply(this,[a.value,b,a])&&(h=!0)),h){k=this.messages[a.name+"."+e]||this.messages[e]||u[e];h="Se ha producido un error con el "+a.display+" field.";k&&(h=k.replace("%s",a.display),b&&(h=h.replace("%s",this.fields[b]?this.fields[b].display:b)));var g;for(b=0;b<this.errors.length;b+=1)a.id===this.errors[b].id&&(g=this.errors[b]);
